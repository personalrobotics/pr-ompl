...
\endverbatim
 */
template <typename KeyType> class heap_indexed {
  struct element {
    KeyType key;
    size_t idx;
...
public:
  ...
  // simple queries
  inline size_t size() const { return backing.size() - 1; }
  ...
  // assumes non-empty
  inline KeyType top_key() const { return backing[1].key; }
  inline size_t top_idx() const { return backing[1].idx; }
